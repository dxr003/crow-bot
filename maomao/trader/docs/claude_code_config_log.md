...
- 2026-04-21 | L0 事件账本正名：`/root/logs/lib/` → `/root/ledger/`，类 `EventLogger` → `Ledger`，接口加 `parent_trace_id` + `cost_usd` 两字段 | 乌鸦「日志升级为事件账本，L0 基础设施，所有模块共用」| 乌鸦「继续开始. 我批」 | （本次 commit）
- 2026-04-21 | scanner.py 封板解锁接入 ledger（进池/退出/否决/信号/买入 5 个事件）+ FileHandler → RotatingFileHandler 20MB×5 | Phase A 信号层接入，trace_id 存进 watchpool 从进池贯穿到买入 | 同批 | （本次 commit）
- 2026-04-21 | dispatch.py 加 dialog/commands.jsonl + set_trace_id 贯穿到 executor | Phase A 对话层接入，场景 B（"2h 前这单哪来的"）验证通过，trace_id `bc157eed` 同时出现在 dialog/exec 两边 | 同批 | （本次 commit）
- 2026-04-21 | guardian.py 接入 system/guardian.jsonl（heartbeat + alert_sent/failed）| Phase A 运维层接入，每次巡检无论有无异常都落一条 | 同批 | （本次 commit）
- 2026-04-21 | ledger.external.log_external_call() 就位 | Phase A 外部 API 记账骨架（analyzer Haiku/Tavily 实际接入放 Phase B，需要解封板+价格表） | 同批 | （本次 commit）
