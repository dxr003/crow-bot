...
## 五、移动止盈预设 v2.0（定稿 2026-04-06）

> 追踪历史最高浮盈，从最高点回撤达阈值触发平仓。容错比 ±3% 防抖。

| 档位 | 指令 | 激活条件 | 浮盈区间 → 回撤触发 |
|------|------|---------|-------------------|
| 1档 保 | `移1` / `保` | ≥10% | ~50%→20% / 50~100%→15% / 100~200%→10% / 200%+→8% |
| 2档 中（默认） | `移2` / `中` | ≥30% | ~50%→30% / 50~100%→20% / 100~200%→15% / 200%+→10% |
| 3档 激 | `移3` / `激` | ≥60% | ~50%→40% / 50~100%→30% / 100~200%→20% / 200%+→15% |

> 旧版四档（conservative/moderate/aggressive/double）已废弃，以此为准。
