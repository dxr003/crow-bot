...
## 五、移动止盈预设（trailing_safe.py）

> 动态攀爬回撤：盈利越高保护越紧。

| 档位 | 激活条件 | 盈利区间 → 回撤触发 |
|------|---------|-------------------|
| `conservative` 保守🛡️ | +10% | 10~50%→5% / 50~100%→4% / 100~200%→3% / 200%+→2% |
| `moderate` 中等⚖️ | +30% | 30~50%→15% / 50~100%→10% / 100~200%→6% / 200%+→3% |
| `aggressive` 激进🔥 | +60% | 60~100%→30% / 100~200%→20% / 200~300%→10% / 300%+→5% |
| `double` 翻倍🚀 | +100% | 100~200%→50% / 200~300%→30% / 300~500%→15% / 500%+→8% |
